...
class Thread : public ThreadInterface
{
	private : 
		int priority_thread;
		unsigned int stackSize_thread;
		Joinable isJoinable_thread;
		const char* name_thread;
		bool isJoignable_thread;
...
||||||| .r1126
		...
		Thread(int priority, unsigned int stackSize, Joinable isJoinable, const char* name = "unnamed") : priority_thread(priority), stackSize_thread(stackSize), isJoinable_thread(isJoinable), name_thread(name), isRunning_thread(false), taskHandle_thread(nullptr) {
=======
		Thread(int priority, unsigned int stackSize, Joinable isJoinable, const char* name = "unnamed") {
>>>>>>> .r1257
            // TODO
...
<<<<<<< .mine
							...
							name_thread = name;
							taskHandle_thread = nullptr; 
			
							if(isJoignable_thread == JOINABLE)
...
<<<<<<< .mine
									if(taskHandle_thread == NULL){
											...
											return false;
||||||| .r1126
			if (isRunning_thread || taskHandle_thread != nullptr ){
